...
NAME
       am2swi: Translating WAM-based Intermediate Code into SWI-Prolog C code!
...
DESCRIPTION
       This program translates WAM-based intermediate codes into SWI_C.
       For each predicate p/n, the file named "PRED_p_n.swi_c" is generated.
       Generated files can be compiled and executed by usual 
       swi_c utilities (ex. swi_cc) with the Prolog Cafe runtime system.

COPYRIGHT
       am2swi (Translating WAM-based Intermediate Code into SWI_C)
...
% :- module('com.googlecode.prolog_cafe.compiler.am2swi', [main/0,am2swi/1]).
package(_). 
:- package 'com.googlecode.prolog_cafe.compiler.am2swi'.
:- public main/0, am2swi/1.
/*****************************************************************
  Main
*****************************************************************/
main :- 
...
write_swi_c(X, _) :- var(X), !, 
	am2swi_error([unbound,variable,is,found]), 
	fail.
write_swi_c(end_of_file, _) :- !.
write_swi_c((:- G), _) :- !, call(G).
write_swi_c(begin_predicate(P, F/A), In) :-
	...
	list_to_string([SrcDir,'/','PRED_',F1,'_',A,'.swi_c'], SrcFile),
	mkdirs(SrcDir),
	...
	repeat,
	  read(In, X),
	  write_swi_c0(X, In, Out),
	X == end_predicate(P, F/A),
	close(Out),
	!.
write_swi_c(X, _) :- 
	am2swi_error([X,is,an,invalid,argument,in,write_swi_c/2]),
	fail.

/*****************************************************************
  Write SWI_C
*****************************************************************/
write_swi_c0(X, _, _) :- var(X), !,
	am2swi_error([unbound,variable,is,found]),
	fail.
write_swi_c0([], _, _) :- !.
write_swi_c0([X|Xs], In, Out) :- !,
	write_swi_c0(X, In, Out),
	write_swi_c0(Xs, In, Out).
write_swi_c0(end_predicate(_, _), _, Out) :- !,
	tab(Out, 4),
	write(Out, '}'), nl(Out),
	write(Out, '}'), nl(Out).
write_swi_c0(comment(Comment), _, Out) :- !,
	numbervars(Comment, 0, _),
	tab(Out, 4), 
	write(Out, '// '), 
	writeq(Out, Comment), nl(Out).
write_swi_c0(debug(Comment), _, Out) :- !,
	numbervars(Comment, 0, _),
	write(Out, '// '), 
	writeq(Out, Comment), nl(Out).
write_swi_c0(info([FA,File|_]), _, Out) :- !,
	write(Out, '/*'), nl(Out),
        ...
        write(Out, '*/'), nl(Out).
write_swi_c0(import_package(P), _, Out) :- !,
	write(Out, 'import '), 
	write_package(P, Out), 
	write(Out, '.*;'), nl(Out).
write_swi_c0(import_package(P,FA), _, Out) :- !,
	write(Out, 'import '), 
	write_package(P, Out), 
	write(Out, '.'), 
	(FA = _/_ -> 
...
write_swi_c0(label(L), _, Out) :- !,
	...
	write(Out, '();'), nl(Out).
write_swi_c0(goto(L), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'return '),
	write_index(L, Out),
	write(Out, ';'), nl(Out).
write_swi_c0(setB0, _, Out) :- !,
	tab(Out, 8),
	write(Out, 'engine.setB0();'), nl(Out).
write_swi_c0(deref(_,void), _, _) :- !.
write_swi_c0(deref(Ri,Rj), _, Out) :- !,
	tab(Out, 8),
	write_reg(Rj, Out),
	write(Out, ' = '),
	write_reg(Ri, Out),
	write(Out, '.dereference();'), nl(Out).
write_swi_c0(set(_,void), _, _) :- !.
write_swi_c0(set(Ri,Rj), _, Out) :- !,
	tab(Out, 8),
	write_reg(Rj, Out),
	write(Out, ' = '),
	write_reg(Ri, Out), 
	write(Out, ';'), nl(Out).
write_swi_c0(decl_term_vars([]), _, _) :- !.
write_swi_c0(decl_term_vars(L), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'Term '),
	write_reg_args(L, Out),
	write(Out, ';'), nl(Out).
write_swi_c0(decl_pred_vars([]), _, _) :- !.
write_swi_c0(decl_pred_vars(L), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'Operation '),
	write_reg_args(L, Out),
	write(Out, ';'), nl(Out).
write_swi_c0(put_cont(BinG,C), _, Out) :- !,
	...
	write(Out, ');'), nl(Out).
write_swi_c0(execute(cont), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'return cont;'), nl(Out).
write_swi_c0(execute(BinG), _, Out) :- !,
	(BinG = P:G -> true ; BinG = G),
	functor(G, F, A0),
	A is A0-1,
	G =.. [F|Args],
	...
	write_reg_args(Args, Out),
	write(Out, ');'), nl(Out).
write_swi_c0(inline(G), In, Out) :- 
	write_inline(G, In, Out),
	!.
write_swi_c0(new_hash(Tag,I), _, Out) :- !,
	tab(Out, 4),
	write(Out, 'static final swi_c.util.HashMap<Term, Operation> '),
	(Tag == int -> write(Out, 'Int') ; write(Out, Tag)),
	write(Out, ' = new swi_c.util.HashMap<Term, Operation>('),
	write(Out, I),
	write(Out, ');'), nl(Out).
write_swi_c0(put_hash(X,L,Tag), _, Out) :- !,
	tab(Out, 8),
	(Tag == int -> write(Out, 'Int') ; write(Out, Tag)),
	write(Out, '.put('),
	write_reg(X, Out),
	...
	write_index(L, Out),
	write(Out, ');'), nl(Out).
write_swi_c0(static(Instrs), In, Out) :- !,
	tab(Out, 4),
	write(Out, 'static {'), nl(Out),
	write_swi_c0(Instrs, In, Out),
	tab(Out, 4),
	write(Out, '}'), nl(Out).
%%% Put Instructions
write_swi_c0(put_var(X), _, Out) :- !,
	tab(Out, 8),
	write_reg(X, Out), 
	write(Out, ' = new VariableTerm(engine);'), nl(Out).
write_swi_c0(put_int(I,X), _, Out) :- !,
	tab(Out, 4),
	write(Out, 'static final IntegerTerm '),
	write_reg(X, Out), 
	write(Out, ' = new IntegerTerm('),
	(swi_c_integer(I) -> true; write(Out, 'new swi_c.math.BigInteger("')),
	write(Out, I), 
	(swi_c_integer(I) -> true; write(Out, '")')),
	write(Out, ');'), nl(Out).
write_swi_c0(put_float(F,X), _, Out) :- !,
	tab(Out, 4),
	...
	write(Out, F),
	write(Out, ');'), nl(Out).
write_swi_c0(put_con(C,X), _, Out) :- !,
	tab(Out, 4),
	...
	nl(Out).
write_swi_c0(put_list(Xi,Xj,Xk), _, Out) :- !,
	(Xk = s(_) -> 
	...
	write_reg(Xj, Out),
	write(Out, ');'), nl(Out).
write_swi_c0(put_str(Xi,Y,Xj), _, Out) :- !,
	(Xj = s(_) -> 
	...
	write_reg(Y, Out),
	write(Out, ');'), nl(Out).
write_swi_c0(put_str_args(Xs,Y), _, Out) :- !,
	(Y = s(_) -> 
	...
	write(Out, '};'), nl(Out).
write_swi_c0(put_clo(G0, X), _, Out) :- !,
	(G0 = P:G -> true ; G0 = G),
	functor(G, F, A),
	G =.. [F|Args0],
	am2swi_append(Args0, ['null'], Args),
	tab(Out, 8),
	write_reg(X, Out),
	...
	write(Out, '));'), nl(Out).
%%% Get Instructions
write_swi_c0(get_val(Xi,Xj), _, Out) :- !,
	tab(Out, 8),
...
%write_swi_c0(get_int(_,Xi,Xj), In, Out) :- !,
%	write_swi_c0(get_val(Xi, Xj), In, Out).
write_swi_c0(get_int(N,Xi,Xj), In, Out) :- !,
	write_swi_c0(deref(Xj,Xj), In, Out),
	% read mode
	tab(Out, 8), 
	...
	tab(Out, 8),
 	write(Out, '}'), nl(Out).
%write_swi_c0(get_float(_,Xi,Xj), In, Out) :- !,
%	write_swi_c0(get_val(Xi, Xj), In, Out).
write_swi_c0(get_float(N,Xi,Xj), In, Out) :- !,
	write_swi_c0(deref(Xj,Xj), In, Out),
	% read mode
	tab(Out, 8), 
	...
	tab(Out, 8),
 	write(Out, '}'), nl(Out).
%write_swi_c0(get_con(_,Xi,Xj), In, Out) :- !,
%	write_swi_c0(get_val(Xi, Xj), In, Out).
write_swi_c0(get_con(_,Xi,Xj), In, Out) :- !,
	write_swi_c0(deref(Xj,Xj), In, Out),
	% read mode
	...
	tab(Out, 8),
 	write(Out, '}'), nl(Out).
write_swi_c0(get_ground(_,Xi,Xj), In, Out) :- !,
	write_swi_c0(get_val(Xi, Xj), In, Out).
write_swi_c0(get_list(X), In, Out) :- !,
	write_swi_c0(deref(X,X), In, Out),
	...
	tab(Out, 8),
 	write(Out, '}'), nl(Out).
write_swi_c0(get_str(_F/A,Xi,Xj), In, Out) :- !,
	write_swi_c0(deref(Xj,Xj), In, Out),
...
write_swi_c0(try(Li,Lj), _, Out) :- !, 
	...
	write_index(Lj, Out),
	write(Out, ');'), nl(Out).
write_swi_c0(retry(Li,Lj), _, Out) :- !, 
	tab(Out, 8),
	...
	write_index(Lj, Out), 
	write(Out, ');'), nl(Out).
write_swi_c0(trust(L), _, Out) :- !, 
	tab(Out, 8),
	write(Out, 'return engine.trust('),
	write_index(L, Out),
	write(Out, ');'), nl(Out).
%%% Indexing Instructions
write_swi_c0(switch_on_term(Lv,Li,Lf,Lc,Ls,Ll), _, Out) :- !, 
	tab(Out, 8),
...
write_swi_c0(switch_on_hash(Tag,_,L, _), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'return engine.switch_on_hash('),
	(Tag == int -> write(Out, 'Int') ; write(Out, Tag)),
	write(Out, ', '),
	write_index(L, Out), write(Out, ');'), nl(Out).
write_swi_c0(Instruction, _, _) :-
	am2swi_error([Instruction,is,an,invalid,instruction]),
	fail.
...
write_unify_r(unify_ground(_,X), N, Out) :- !,
	write_unify_r(unify_val(X), N, Out).
write_unify_r(X, _, _) :-
	am2swi_error([X,is,an,invalid,instruction]),
	fail.
...
write_inline0(swi_c(X), _, Out)    :- !, write_if_fail(op('!', @('isSWI_CObject'(X))), [X], 8, Out).
...
write_inline0(swi_c(X,Y), _, Out) :- !, 
	write_if_fail(op('!', @('isSWI_CObject'(X))), [X], 8, Out),
	EXP = #('SymbolTerm.makeSymbol'(@(getName(@(getClass(@(object(cast('SWI_CObjectTerm',X))))))))),
...
write_inline_swi_c(X, _, _) :- var(X), !, fail.
write_inline_swi_c([], _, _) :- !.
write_inline_swi_c([X|Xs], Tab, Out) :- !,
	write_inline_swi_c(X, Tab, Out),
	write_inline_swi_c(Xs, Tab, Out).
write_inline_swi_c(try_catch(TRY,EXCEPT,CATCH), Tab, Out) :- !,
	tab(Out, Tab),
	write(Out, 'try {'), nl(Out),
	Tab1 is Tab + 4,
	write_inline_swi_c(TRY, Tab1, Out),
	tab(Out, Tab),
	write(Out, '} catch ('), write(Out, EXCEPT), write(Out, ' e) {'), nl(Out),
	write_inline_swi_c(CATCH, Tab1, Out),
	tab(Out, Tab),
	write(Out, '}'), nl(Out).
write_inline_swi_c(if_then(IF, THEN), Tab, Out) :- !,
	tab(Out, Tab),
	write(Out, 'if ('), write_inline_exp(IF, 0, Out), write(Out, ') {'), nl(Out),
	Tab1 is Tab + 4,
	write_inline_swi_c(THEN, Tab1, Out),
	tab(Out, Tab),
	write(Out, '}'), nl(Out).
write_inline_swi_c(if_then_else(IF, THEN, ELSE), Tab, Out) :- !,
	tab(Out, Tab),
	write(Out, 'if ('), write_inline_exp(IF, 0, Out), write(Out, ') {'), nl(Out),
	Tab1 is Tab + 4,
	write_inline_swi_c(THEN, Tab1, Out),
	tab(Out, Tab),
	write(Out, '} else {'), nl(Out),
	write_inline_swi_c(ELSE, Tab1, Out),
	tab(Out, Tab),
	write(Out, '}'), nl(Out).
write_inline_swi_c(X, Tab, Out) :-
	tab(Out, Tab),
...
% int
swi_c_integer(X) :- integer(X), -2147483648 =< X, X =< 2147483647.
...
pred_encoding_char(X) --> {0 =< X, X =< 65535}, !, 
	...
	pred_encoding_hex(X).
pred_encoding_char(X) --> 
	{am2swi_error([X,is,an,invalid,character,code]), fail}.
...
% Write Register name
write_reg(X, _) :- var(X), !,
	am2swi_error([register,expression,must,not,be,unbound,variable]),
	fail.
...
write_reg(X, _) :-
	am2swi_error([X,is,an,invalid,register,expression]),
	fail.
...
%%% print
am2swi_error(M) :- am2swi_message(['***','AM2JAVA','ERROR'|M]).

am2swi_message([]) :- nl, flush_output(user_output).
am2swi_message([M|Ms]) :- write(M), write(' '), am2swi_message(Ms).

%%% list
am2swi_append([], Zs, Zs).
am2swi_append([X|Xs], Ys, [X|Zs]) :- am2swi_append(Xs, Ys, Zs).
